...
Widget buildInfoSessionList(context) {
    ...
    final allCheckedIn = Provider.of<AppState>(context, listen: true).checkedInSessions ?? <CalEvent>{};
    print("Checked-in sessions available: ${allCheckedIn.map((e) => e.id)}");
    final checkedInInfoSessions = allCheckedIn.where((event) => event.eventType == "infoSession").toList();
    final List<CalEvent> future = checkedInInfoSessions.where((event)=>(event.startTime.isAfter(now))||(event.startTime.isAtSameMomentAs(now))).toList();
    ...
    List<CalEvent> past = checkedInInfoSessions.where((event)=>(event.startTime.isBefore(now))&&event.startTime.isAfter(pastDays)).toList();
    ...
    List<Widget> buildSection(
        String sectionTitle, List<CalEvent> items) {
      if (items.isEmpty) return [sectionHeader("No " + sectionTitle + " Info Sessions.")];
    ...
    return (future.isNotEmpty||past.isNotEmpty) ? ListView(
    ...
    sectionHeader(
      "You are not checked in to any info sessions."
    );
  }
